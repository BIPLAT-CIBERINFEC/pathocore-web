...
python3 deployment/lib/container/configure_settings.py \
  --answers deployment/settings/configure_answers_settings.ini \
  --settings conf/docker_production_settings.txt,deployment/settings/pathocore_web_production_settings.txt \
  --settings ../pathocore-api/conf/docker_production_settings.txt,deployment/settings/pathocore_api_production_settings.txt \
  --settings ../mepram-omop-api/conf/docker_production_settings.txt,deployment/settings/mepram_omop_api_production_settings.txt \
  --settings conf/apache/apache_production_settings.txt,deployment/settings/apache_production_settings.txt \
  --settings conf/keycloak/keycloak_production_settings.txt,deployment/settings/keycloak_production_settings.txt
```
...
`configure_settings.py` crea las copias que falten bajo `deployment/settings/`
con modo `0600`, pregunta una sola vez cada dato compartido `CHANGE_ME{NOMBRE}`,
genera sin mostrarlos los secretos propios del despliegue y guarda las
respuestas no secretas en `deployment/settings/configure_answers_settings.ini`.
Con `--non-interactive` falla si falta alguna respuesta. Puede repetirse: no
modifica valores ya definidos y lista los ajustes anadidos o eliminados en las
plantillas, que deben revisarse antes de cada actualizacion.

Revisar despues unicamente las copias bajo `deployment/settings/`, completar
todas esas decisiones y confirmar que no queda ningun `CHANGE_ME` antes de
continuar. Los comandos de instalacion y actualizacion usan estas rutas
protegidas.
...
servicios con un namespace de host distinto al despliegue principal. This must be 
executed as `root`, sudo user is not enough.
