...
PATHOCORE_API_LOCAL_SERVER_IP='${PATHOCORE_API_LOCAL_SERVER_IP:-127.0.0.1}'
PATHOCORE_API_DNS_URL='${PATHOCORE_API_DNS_URL:-localhost}'
DJANGO_DEBUG='${DJANGO_DEBUG:-false}'

EMAIL_HOST='${EMAIL_HOST:-${EMAIL_HOST_SERVER:-localhost}}'
EMAIL_HOST_SERVER='${EMAIL_HOST_SERVER:-${EMAIL_HOST:-localhost}}'
...
KEYCLOAK_REALM='${KEYCLOAK_REALM:-ciberisciii_datahub}'
...
KEYCLOAK_ADMIN_TOKEN_REALM='${KEYCLOAK_ADMIN_TOKEN_REALM:-master}'
KEYCLOAK_ADMIN_CLIENT_ID='${KEYCLOAK_ADMIN_CLIENT_ID:-admin-cli}'
KEYCLOAK_ADMIN_CLIENT_SECRET='${KEYCLOAK_ADMIN_CLIENT_SECRET:-}'
KEYCLOAK_ADMIN_USERNAME='${KEYCLOAK_ADMIN_USERNAME:-}'
KEYCLOAK_ADMIN_PASSWORD='${KEYCLOAK_ADMIN_PASSWORD:-}'
KEYCLOAK_ADMIN_REQUEST_TIMEOUT_SECONDS=${KEYCLOAK_ADMIN_REQUEST_TIMEOUT_SECONDS:-10}
KEYCLOAK_ADMIN_SEND_ACTION_EMAILS='${KEYCLOAK_ADMIN_SEND_ACTION_EMAILS:-true}'
KEYCLOAK_ADMIN_ACTION_EMAIL_REDIRECT_URI='${KEYCLOAK_ADMIN_ACTION_EMAIL_REDIRECT_URI:-}'
PATHOCORE_ACCESS_REQUEST_ADMIN_EMAILS='${PATHOCORE_ACCESS_REQUEST_ADMIN_EMAILS:-}'
...
start_app() {
    ...
    exec su -s /bin/bash "${APP_USER_NAME}" -c "cd '${APP_INSTALL_PATH}' && source virtualenv/bin/activate && exec gunicorn pathocore_api.wsgi:application --bind 0.0.0.0:${APP_PORT} --workers ${WEB_CONCURRENCY:-${GUNICORN_WORKERS:-2}} --threads ${GUNICORN_THREADS:-2} --timeout ${GUNICORN_TIMEOUT:-120} --keep-alive ${GUNICORN_KEEPALIVE:-5} --access-logfile - --error-logfile - --capture-output --log-level ${GUNICORN_LOG_LEVEL:-info}"
}
