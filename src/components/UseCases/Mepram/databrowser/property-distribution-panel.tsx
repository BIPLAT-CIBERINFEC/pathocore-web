  ...
  return (
    <Card className="border-white/70 bg-white/88">
      <CardHeader className="pb-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              {card.description ??
                "Territorial distribution of samples with this property."}
            </p>
          </div>
      ...
      <CardContent className="space-y-5">
        <BarChartPanel
          data={chartData}
          description="Samples with this property by location."
          title="Location distribution"
        />
        <div className="rounded-[1.1rem] border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
          Geographic note: entries that include{" "}
          <span className="font-mono">geo.code</span>,{" "}
          <span className="font-mono">lat</span> o{" "}
          <span className="font-mono">lon</span> should be rendered as a map
          when the cartographic layer is integrated; this panel keeps bars as a
          fallback.
        </div>
              ...
              {breakdown.values.map((item) => (
                <tr key={`${item.label}-${item.geo?.code ?? "no-code"}`}>
                  ...
                  <td className="px-4 py-3">
                    {formatInteger(item.matched_samples)}
                  </td>
                  <td className="px-4 py-3">
                    {formatInteger(item.total_samples)}
                  </td>
                  <td className="px-4 py-3">
                    {formatShare(item.matched_share)}
                  </td>
                </tr>
              ))}
            </tbody>
