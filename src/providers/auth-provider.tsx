...
import { setAuthData } from "@/lib/features/authSlice";
...
const AUTH_SESSION_KEY = "mepram.keycloak.session";
const PKCE_SESSION_KEY = "mepram.keycloak.pkce";
...
function keycloakRealmUrl() {
  return `${KEYCLOAK_URL.replace(/\/$/, "")}/realms/${encodeURIComponent(
    KEYCLOAK_REALM
  )}`;
}
...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    if (!code || !state) {
      setError(
        "Keycloak callback is missing the authorization code. Start login again."
      );
      setStatus("error");
      return;
    }
