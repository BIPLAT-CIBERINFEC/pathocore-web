...
Create the protected settings for every production template this topology
consumes. `configure_settings.py` copies each missing file to
`deployment/settings/` with mode `0600`, asks once for every shared
`CHANGE_ME{NAME}` input (public DNS names, Keycloak realm and client, SMTP,
database names), generates deployment-owned secrets without printing them, and
saves non-secret answers for reuse:
...
Add `--non-interactive` to fail instead of prompting when the answers file is
incomplete. Re-running is safe: values already set are kept, only remaining
placeholders are filled, and settings added to or removed from the templates
since the last run are listed for the upgrade configuration review. Review the
remaining values under `deployment/settings/` (paths, UID/GID, ports) and keep
their mode at `0600`. Both installation workflows below point to these
protected copies.
