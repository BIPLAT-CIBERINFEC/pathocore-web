...
permission_services=(pathocore_web pathocore_api mepram_omop_api pathocore-web-apache pathocore-web-keycloak-db pathocore-web-keycloak)
...
for service_name in "${configured_services[@]}"; do
    ...
    if [ "$mode" = production ] \
        ...
        die "Production configuration for $service_name contains CHANGE_ME: $path (complete it with deployment/lib/container/configure_settings.py)"
    fi
