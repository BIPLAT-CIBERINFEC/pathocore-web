...
Rules:
  ...
  derived-setting         a proxy target is set although Compose now derives it
...
def check_derived_settings(
    environment: dict[str, str], compose_text: str
) -> tuple[dict[str, str], list[Finding]]:
    """Report proxy targets that Compose no longer reads and drop them.

    Compose generates <UPSTREAM>_PROXY_TARGET from SERVICES.<name>.UPSTREAMS.
    A value left in an operator settings file is ignored, so it is removed
    before the remaining checks and reported for deletion.
    """
    findings: list[Finding] = []
    used: dict[str, str] = {}
    for key, value in environment.items():
        if key.endswith("_PROXY_TARGET") and "${" + key not in compose_text:
            findings.append(Finding(
                "derived-setting",
                f"{key} is ignored: Compose derives proxy targets from UPSTREAMS; "
...
def run_checks(
    ...
    if compose_text is not None:
        environment, derived_findings = check_derived_settings(environment, compose_text)
...
def main() -> int:
    ...
    findings = run_checks(
        environment, compose, profiles, routes, args.mode,
        args.compose_file.read_text(),
    )
    errors = [
